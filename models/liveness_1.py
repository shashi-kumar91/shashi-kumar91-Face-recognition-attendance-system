...
class MLVSDetector:
    ...
    MIN_FRAMES  = 12
    FPS_EST     = 8.0
    LIVE_RATIO  = 0.20      # RELAXED: was 0.35, easier pass for real faces
    SPOOF_RATIO = 0.02      # RELAXED: was 0.08, less aggressive spoof detection
...
class _RPPGDetector:
    ...
    Real face: variance > 0.20. Photo: < 0.05. Screen: 0.05-0.15.
    Reference: de Haan & Jeanne, IEEE TBME 2013.
    """
    MIN_FRAMES = 15
    WINDOW     = 30
    LIVE_VAR   = 0.10      # RELAXED: was 0.20, accommodate lower quality webcams
    SPOOF_VAR  = 0.02      # RELAXED: was 0.05, less aggressive detection
...
_EAR_TH       = 0.28       # RELAXED: was 0.22, easier blink detection
_TURN_YAW_TH  = 15.0       # RELAXED: was 20.0, smaller head turn required
_NOD_PITCH_TH = 12.0       # RELAXED: was 15.0, smaller nod required
...
class PACEEngine:
    ...
    Zone 1 (CNN > 0.72): Trusted — no challenge (~1 s decision)
    Zone 2 (CNN 0.50-0.72): Light — one blink, 12 s window
    Zone 3 (CNN 0.30-0.50): Heavy — blink + head move, 18 s window
    Zone 4 (CNN < 0.30): Hard reject — no challenge offered
    ...
    """
    ZONE1 = 0.60      # RELAXED: was 0.72
    ZONE2 = 0.45      # RELAXED: was 0.50
    ZONE3 = 0.30
    T2    = 15.0      # EXTENDED: was 12 seconds, more time for blink
    T3    = 20.0      # EXTENDED: was 18 seconds, more time for head movement
...
class LivenessChecker:
    ...
    Decision (patient):
      ...
      CNN > 0.72, not static → True  (Zone 1 fast pass)
      CNN < 0.25           → False (hard reject)
      Completely static    → False
      CNN 0.50+ + MLVS live → True  (2-cue confirm)
      CNN 0.50+ + rPPG live → True  (2-cue confirm)
      > 20 frames, uncertain → PACE Zone 2 or 3
    """

    CNN_WARMUP      = 8
    CNN_ZONE1       = 0.60      # RELAXED: was 0.72, easier fast-pass
    CNN_HARD_FAIL   = 0.20      # RELAXED: was 0.25, more tolerant
    PACE_AFTER      = 25        # EXTENDED: was 20, give more frames to verify
    ...
    def check(self,
        ...
        if static and self._n >= 12:
            logger.warning("Static image guard triggered")
            return False

        # ── Zone 1: trusted (no challenge needed) ────────────────────────
        if cnn >= self.CNN_ZONE1 and not static:
            logger.info(f"Zone 1 fast-pass: CNN={cnn:.3f}")
            return True

        # ── 2-cue live confirms (RELAXED thresholds) ────────────────────
        if cnn >= 0.40 and mlvs is True:      # RELAXED: was 0.50
            logger.info(f"MLVS confirm: CNN={cnn:.3f}")
            return True

        if cnn >= 0.40 and rppg is True:      # RELAXED: was 0.50
            logger.info(f"rPPG confirm: CNN={cnn:.3f}")
            return True

        # ── 2-cue spoof confirms (VERY CONSERVATIVE now) ────────────────
        # Only hard-reject if BOTH liveness detectors agree it's a spoof
        # AND CNN is very low. Single detector can't reject.
        if mlvs is False and rppg is False and cnn < 0.35:  # RELAXED: was cnn < 0.60
            logger.warning("MLVS+rPPG double spoof signal + very low CNN")
            return False
